...
describe('ACTION_RULES', () => {
  ...
  it('assassinate costs match GAME_CONSTANTS', () => {
    expect(ACTION_RULES[ActionType.ASSASSINATE].cost).toBe(GAME_CONSTANTS.ASSASSINATE_COST);
  });
  ...
  it('duke blocks foreign aid', () => {
    expect(ACTION_RULES[ActionType.FOREIGN_AID].blockedBy).toContain(Character.DUKE);
  });

  it('contessa blocks assassination', () => {
    expect(ACTION_RULES[ActionType.ASSASSINATE].blockedBy).toContain(Character.CONTESSA);
  });

  it('captain and ambassador block stealing', () => {
    expect(ACTION_RULES[ActionType.STEAL].blockedBy).toContain(Character.CAPTAIN);
    expect(ACTION_RULES[ActionType.STEAL].blockedBy).toContain(Character.AMBASSADOR);
  });
