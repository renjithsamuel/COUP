...
export function LobbyRoom({ lobby, myPlayerId, isHost, leaderboard, onStart, onKick, onLeave }: LobbyRoomProps) {
  ...
  const confirmationModal = confirmState && typeof document !== 'undefined'
    ? createPortal(
      <div style={s.modalOverlay} onClick={() => !isConfirming && setConfirmState(null)}>
        <motion.div
          style={s.confirmCard}
        ...
        >
          <div style={s.confirmCopy}>
            <span style={s.sectionEyebrow}>{confirmState.type === 'leave' ? 'Leave room' : 'Remove player'}</span>
          ...
          <div style={s.confirmActions}>
            <button type="button" style={s.secondaryButton} onClick={() => setConfirmState(null)} disabled={isConfirming}>
              Cancel
            </button>
            <button
              type="button"
            ...
            >
              {isConfirming ? 'Working...' : confirmState.type === 'leave' ? 'Leave room' : 'Kick player'}
            </button>
          </div>
        </motion.div>
      </div>,
  ...
  const leaderboardModal = showLeaderboard && typeof document !== 'undefined'
    ? createPortal(
          ...
          <div style={s.modalHeader}>
            <div style={s.modalHeaderCopy}>
              ...
              <div style={s.modalRoomMeta}>
                <span style={s.modalRoomName}>{lobby.name || 'Untitled room'}</span>
                <span style={s.modalRoomCode}>{lobby.id}</span>
              </div>
              <span style={s.modalSubtitle}>Only players who have played in this room appear here. Scores expire with the room history after the configured grace period.</span>
            </div>
          ...
          {leaderboardRows.length > 0 ? (
            <div style={s.leaderboardList}>
              {leaderboardRows.map((entry) => (
                <div key={`${entry.playerKey}-${entry.rank}`} style={s.leaderboardRow}>
            ...
            <div style={s.emptyLeaderboard}>
              No completed games for this room yet. Finish the first round and these room-only scores will appear here.
            </div>
          )}
        </motion.div>
      </div>,
        ...
        {lobby.players.map((player, i) => (
          ...
          >
            ...
            <div style={s.playerActions}>
              ...
              {player.id !== myPlayerId && onKick && lobby.status === 'waiting' && (
                <button
                  type="button"
                  style={s.kickButton}
                  onClick={() => setConfirmState({ type: 'kick', playerId: player.id, playerName: player.name })}
                  disabled={kickingPlayerId != null || isConfirming}
                >
                  <span style={s.buttonIcon}><KickIcon /></span>
                  <span>{kickingPlayerId === player.id ? 'Removing...' : 'Kick'}</span>
                </button>
              )}
            </div>
          </motion.div>
      ...
      <div style={s.footer}>
        <div style={s.footerActions}>
          <button style={s.secondaryButton} onClick={() => setShowLeaderboard(true)}>
            <span style={s.buttonIcon}><LeaderboardIcon /></span>
          ...
          {onLeave && (
            ...
            >
              <span style={s.buttonIcon}><LeaveIcon /></span>
