...
export function GameDashboard({ gameState, myPlayerId, isMobile = false }: GameDashboardProps) {
    ...
    <div style={styles.wrapper(isMobile)}>
      <div style={styles.summaryRow(isMobile)}>
        <div style={styles.summaryCard(isMobile)}>
          ...
          <span style={styles.summaryValue(isMobile)}>{gameState.turnNumber}</span>
        </div>
        <div style={styles.summaryCard(isMobile)}>
          <span style={styles.summaryLabel(isMobile)}>Deck</span>
          <span style={styles.summaryValue(isMobile)}>{gameState.deckSize} cards</span>
        </div>
        <div style={styles.summaryCard(isMobile)}>
          <span style={styles.summaryLabel(isMobile)}>Alive</span>
          <span style={styles.summaryValue(isMobile)}>{playersAlive} players</span>
        </div>
      </div>
      ...
      <div style={styles.playerList(isMobile)}>
        {sortedPlayers.map((player, index) => {
          ...
          return (
            <div key={player.id} style={styles.playerCard(isCurrent, isMe, isMobile)}>
              {isMobile ? (
                <>
                  <div style={styles.mobileTopRow}>
                    <div style={styles.playerIdentity}>
                      ...
                      <div style={styles.playerCopy}>
                        <div style={styles.playerNameRow}>
                          <span style={styles.playerName(isMe, isMobile)}>{player.name}</span>
                          {isMe && <span style={styles.softBadge(isMobile)}>You</span>}
                          {isCurrent && <span style={styles.accentBadge(isMobile)}>Turn</span>}
                        </div>
                        {player.revealedCards.length > 0 && (
                          <div style={styles.revealedInlineWrap}>
                            {player.revealedCards.map((card, i) => (
                              <span key={`${player.id}-${i}`} style={styles.revealedCard(isMobile)}>
                                {card.character}
                              </span>
                            ))}
                          </div>
                    ...
                    <div style={styles.statusCellMobile}>
                      <span style={player.isAlive ? styles.aliveBadge(isMobile) : styles.eliminatedBadge(isMobile)}>
                  ...
                  <div style={styles.mobileStatGrid}>
                    <div style={styles.mobileStatCard}>
                      ...
                      <span style={styles.mobileStatValueAccent}>{player.coins}</span>
                    </div>
                    <div style={styles.mobileStatCard}>
                      <span style={styles.mobileStatLabel}>Inf</span>
                      <span style={styles.mobileStatValue}>{player.influenceCount}</span>
                    </div>
                    <div style={styles.mobileStatCard}>
                      <span style={styles.mobileStatLabel}>Rev</span>
                      <span style={styles.mobileStatValue}>{player.revealedCards.length}</span>
                    </div>
                  </div>
                </>
              ) : (
...
const styles = {
  ...
  playerCard: (isCurrent: boolean, isMe: boolean, isMobile: boolean) => ({
    ...
    gridTemplateColumns: isMobile ? undefined : 'minmax(0, 1.7fr) 68px 52px 52px 80px',
    gap: isMobile ? 8 : 8,
    alignItems: 'center',
  }),
  mobileTopRow: {
