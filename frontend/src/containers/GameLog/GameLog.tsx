...
function getEntryVisual(
  type: string,
  actionType?: keyof typeof ACTION_PRESENTATIONS,
) {
  ...
  switch (type) {
    case "challenge":
      return {
        accent: "#F3C969",
        tint: "rgba(243, 201, 105, 0.14)",
        icon: "challenge" as const,
      };
    case "block":
      return {
        accent: "#B693FF",
        tint: "rgba(182, 147, 255, 0.14)",
        icon: "block" as const,
      };
    case "reveal":
      return {
        accent: "#F08A94",
        tint: "rgba(240, 138, 148, 0.14)",
        icon: "reveal" as const,
      };
    case "elimination":
      return {
        accent: "#F06B7D",
        tint: "rgba(240, 107, 125, 0.14)",
        icon: "elimination" as const,
      };
    case "turn":
      return {
        accent: "#75D6A9",
        tint: "rgba(117, 214, 169, 0.14)",
        icon: "turn" as const,
      };
    default:
      return {
        accent: "#F6C445",
        tint: "rgba(246, 196, 69, 0.12)",
...
export function GameLog({ variant = "modal" }: GameLogProps) {
  ...
  return (
    <div style={gameLogStyles.container(variant)}>
      <div
        ref={scrollRef}
        style={gameLogStyles.wrapper(variant)}
      ...
      >
        ...
        {entries.map((entry, index) => {
          ...
          const actionLabel = entry.actionType
            ? ACTION_RULES[entry.actionType].label
            : null;
            ...
            <article key={entry.id} style={gameLogStyles.entryRow}>
              ...
              <div style={gameLogStyles.messageColumn(visual, variant)}>
                ...
                <div style={gameLogStyles.messageStack}>
                  {actionLabel && (
                    <span style={gameLogStyles.actionPill(visual)}>
                      {actionLabel}
                    </span>
                  )}
