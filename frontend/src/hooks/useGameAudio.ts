...
const turnProfile: AudioProfile = {
  notes: [523.25, 659.25],
  duration: 0.08,
  gain: 0.013,
  type: "sine",
...
export function useGameAudio() {
  ...
  const playTurnSound = useCallback(async () => {
    ...
    const now = audioContext.currentTime;
    turnProfile.notes.forEach((frequency, index) => {
      const oscillator = audioContext.createOscillator();
      const envelope = audioContext.createGain();
      const filter = audioContext.createBiquadFilter();
      const startAt = now + index * 0.055;
      const stopAt = startAt + turnProfile.duration;

      oscillator.type = turnProfile.type;
      oscillator.frequency.setValueAtTime(frequency, startAt);
      filter.type = "lowpass";
      filter.frequency.setValueAtTime(3200, startAt);
      filter.Q.setValueAtTime(0.18, startAt);
